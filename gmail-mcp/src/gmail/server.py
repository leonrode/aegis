...
class GmailService:
    def __init__(self,
        ...
        self.creds_file_path = os.path.join(os.getcwd(), creds_file_path)
        self.token_path = os.path.join(os.getcwd(), token_path)
    ...
    def _get_token(self) -> Credentials:
        ...
        if not token or not token.valid:
            if token and token.expired and token.refresh_token:
                ...
                try:
                    token = flow.run_local_server(port=8080, prompt="consent")
                except OSError as e:
                    if "Address already in use" in str(e):
                        logger.info("Port 8080 is in use, trying port 8081...")
                        try:
                            token = flow.run_local_server(port=8081, prompt="consent")
                        except OSError:
                            logger.info("Port 8081 is in use, trying port 8082...")
                            token = flow.run_local_server(port=8082, prompt="consent")
                    else:
